...
const CountriesList = forwardRef((props, ref) => {
  ...
  const { countries, isLoading, isError, onCountrySelected, rows } = props;
  ...
  const elements = countries.map(item => <CountriesListItem key={item.name} onClick={() => onCountrySelected(item.name)} {...item} />);

  return (
    <ul 
      ref={ref}
      style={{ gridTemplateRows: `repeat(${rows}, minmax(126px, auto))` }} 
      className={`${styles.root} ${isLoading ? styles.loading : ''}`}
    >
      {elements}
    </ul>
  );
